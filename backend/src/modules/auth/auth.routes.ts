...
authRouter.post('/register', async (req, res, next) => {
  try {
    ...
    if (!isFirstUser && !env.ALLOW_PUBLIC_REGISTRATION) return res.status(403).json({ code: 'REGISTRATION_DISABLED', message: 'Public registration is disabled. Contact the administrator.' })
    const user = await prisma.user.create({ data: { name: body.name, email: body.email, passwordHash: await hashPassword(body.password), role: isFirstUser ? 'admin' : 'user' } })
...
authRouter.get('/google/callback', async (req, res) => {
  try {
    ...
    if (!existingUser) {
      const userCount = await prisma.user.count()
      const isFirstUser = userCount === 0
      if (!isFirstUser && !env.ALLOW_PUBLIC_REGISTRATION) return res.redirect(`${env.FRONTEND_URL}/google-auth?status=registration_disabled`)
    }
    const userCount = existingUser ? 1 : await prisma.user.count()
    const isFirstUser = !existingUser && userCount === 0
    const user = await prisma.user.upsert({
      where: { email },
      create: { email, name, passwordHash: await hashPassword(randomToken(32)), role: isFirstUser ? 'admin' : 'user' },
      update: { name },
    })
...
authRouter.get('/me', requireAuth, async (req: AuthRequest, res, next) => {
  try {
    const user = await prisma.user.findUniqueOrThrow({ where: { id: req.user!.id }, select: { id: true, name: true, email: true, status: true, role: true } })
